...
# cmn_ask_to_confirm message
#
# Asks the user - using the given message - to either hit 'y/Y' to continue or
# 'n/N' return false.
# In case of 'n/N', the rest of the line will not confinue.
#
# Example:
# cmn_ask_to_confirm "Create user?" && useradd user
#
function cmn_ask_to_confirm {
  local msg=${1}
  local properanswer=true
  while ${properanswer}; do
    read -r -p "${msg} (hit 'y/Y' to confirm, 'n/N' to cancel) " -n 1 ynanswer
    case ${ynanswer} in
      [Yy] ) echo ""; echo "Proceeding..."; return 0;;
      [Nn] ) echo ""; echo "Skipping..."; return -1;;
      *    ) echo ""; echo "Please answer either yes (y/Y) or no (n/N).";;
    esac
  done
}
